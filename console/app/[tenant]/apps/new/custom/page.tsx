...
export default async function NewAppCustomPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-[color:var(--color-muted)] mb-6">
        Provision an ArgoCD Application for{" "}
        <span className="font-medium text-[color:var(--color-fg)]">{tenant.name}</span>{" "}
        from any Helm chart. Submitting writes two files to git{" "}
        {configured ? (
          <>
      ...
      <form action={provisionAppFromForm} className="flex flex-col gap-4">
        ...
        <input type="hidden" name="_returnTo" value={`/${slug}/apps/new/custom`} />
